...
public class LinePagerIndicator extends View implements IPagerIndicator {
    ...
    public static final int MODE_WRAP_CONTENT_2 = 3;    // 直线宽度 == title内容宽度 - 2 * mXOffset
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        } else if (mMode == MODE_WRAP_CONTENT_2) {
            //
            leftX = current.mContentLeft + mXOffset - offset;
            nextLeftX = next.mContentLeft + mXOffset - offset;
            rightX = current.mContentRight - mXOffset + offset ;
            nextRightX = next.mContentRight - mXOffset + offset;
        } else {    // MODE_EXACTLY
    ...
    public void setMode(int mode) {
        if (mode == MODE_EXACTLY || mode == MODE_MATCH_EDGE || mode == MODE_WRAP_CONTENT || mode == MODE_WRAP_CONTENT_2) {
            mMode = mode;
        } else {
